...
class WorkFlowItem extends Component<WorkFlowItemProps, State> {
    ...
    render() {
        ...
        return <div>
            <div ref={(element: HTMLDivElement) => this.container = element} className="workflow-item-container" id={this.props.workflowId} />
            <Drawer title='编辑表单' placement='right' visible={visible} onClose={() => this.closeDrawer()} >
                <WorkflowForm createOrUpdateNode={this.createOrUpdateNode} data={currentSelectedNodeData} />
            </Drawer>
        </div>;
    }
}
