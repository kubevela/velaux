...
export function checkImageName(query: { image: string; project: string }) {
  const urlPath = project + `/${query.project}/validate_image`;
  return get(urlPath, { params: { image: query.image } }).then((res) => res);
}
