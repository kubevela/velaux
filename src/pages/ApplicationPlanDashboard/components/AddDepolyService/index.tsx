...
class addCheckDepolySercice extends Component<Props, State>{
    constructor(props: Props) {
        super(props);
        this.state = {
            componentServices: (props.components || []).map((item: ComponentItem) => ({ id: item.name, name: item.name, checked: true })),
            allChecked: true,
        }
    }
    ...
    onChangeAll = (flag: boolean) => {
        const { componentServices } = this.state;
        if (flag) {
            const newComponentServices = componentServices.map((item: ComponentItem) => ({ id: item.name, name: item.name, checked: true }));
            this.setState({ componentServices: newComponentServices, allChecked: true });
        } else {
            const newComponentServices = componentServices.map((item: ComponentItem) => ({ id: item.name, name: item.name, checked: false }));
                        ...
                        componentServices.map((item: ComponentItem) => (
                            <Col span='6'>
                                <Checkbox
                                    ...
                                    onChange={(flag, e) => { this.onChange(flag, e) }}>
                                    <label htmlFor={item.name} className="next-checkbox-label">{item.name}</label>
                                </Checkbox>
                            </Col>
                        )
                        )
