...
import type { CloudShellPrepare } from '@velaux/data';
...
type State = {
  preparePadding: boolean;
  prepare?: CloudShellPrepare;
};
@connect((store: any) => {
  return { ...store.cloudshell };
})
class CloudShell extends Component<Props, State> {
  ...
  prepare = () => {
    prepareCloudShell().then((res: CloudShellPrepare) => {
  ...
  renderPrepareView() {
    ...
    return (
      <div className="prepare">
        ...
        <span style={{ marginTop: '8px', fontWeight: 500 }}>Prepare The Cloud Shell</span>
        <span style={{ marginTop: '8px' }}>{message}</span>
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <If condition={preparePadding}>{this.renderPrepareView()}</If>
        <If condition={!preparePadding}>{this.renderCloudShellView()}</If>
      </Drawer>
    );
  }
}
